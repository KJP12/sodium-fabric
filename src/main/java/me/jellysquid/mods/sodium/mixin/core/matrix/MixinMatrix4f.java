...
public class MixinMatrix4f implements Matrix4fExtended {
    ...
    // FIXME: weird shit is probably about to happen
    /**
     * @reason Optimize
     * @author KJP12; Adapted from JellySquid's code.
     */
...
//
        if (buf.isDirect() && UnsafeUtil.isAvailable()) {
            this.writeColumnMajorToBufferUnsafe(buf);
        } else {
            this.writeColumnMajorToBufferSafe(buf);
        }
    }
    ...
    public void writeRowMajor(FloatBuffer buf) {
        ...
        if (buf.isDirect() && UnsafeUtil.isAvailable()) {
            this.writeRowMajorToBufferUnsafe(buf);
        } else {
            this.writeRowMajorToBufferSafe(buf);
        }
    }

    private void writeColumnMajorToBufferUnsafe(FloatBuffer buf) {
    ...
    private void writeColumnMajorToBufferSafe(FloatBuffer buf) {
        buf.put(0, this.a00);
        buf.put(4, this.a01);
        buf.put(8, this.a02);
        buf.put(12, this.a03);
    ...
    private void writeRowMajorToBufferUnsafe(FloatBuffer buf) {
        long addr = MemoryUtil.memAddress(buf);
